...
function formatCurrency(amount: number): string{
  ...
  if(fractionalPart*10 > 4){
    kopeika='копеек'
  }

  return `${amount.toFixed(2)} (${integerWords} белорусских рубля ${fractionalWords} ${kopeika})`;
}

document.querySelector('.main__change-button')?.addEventListener('click', () => {
  const inputElement = document.querySelector<HTMLInputElement>('.main__past-number');
  const outputElement = document.querySelector<HTMLDivElement>('.main__word-number');
